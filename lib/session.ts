...
export async function getSession(): Promise<SessionUser | null> {
  ...
  try {
    ...
    const result = await sql`SELECT id, name, is_admin FROM users WHERE id = ${id} LIMIT 1`;
    const user = result.rows[0] as { id: number; name: string; is_admin: number } | undefined;
    if (!user) return null;
